...
int main() {
    ...
    while (tC--) {
        ...
        map<int,int> nodoFila;
        map<int,int> nodoColumna; 
        int fila = 0, columna = 0;
        for (int i = 0; i < n; i++) {
            int a, b; cin>>a>>b;
            if (nodoFila.find(a) == nodoFila.end()) nodoFila[a] = fila++;
            if (nodoColumna.find(b) == nodoColumna.end()) nodoColumna[b] = columna++;
            h.add_edge(nodoFila[a], nodoColumna[b]);
        }
