...
void dfs(map<int, pair<int,int>>& mapa, vector<ll>& count, int v, int p = 1, int nivel = 1) {
    if (maes[v]) {
        if (mapa[nivel].first == 0) mapa[nivel].first = v;
        mapa[nivel].second = v;
        count[nivel] += maes[v];
    }
    for (int u : adj[v]) {
        if (u == p) continue;
        dfs(mapa, count, u, v, nivel+1);
    }
}
...
int main() {
    ...
    for(int u : adj[1]) {
        ...
        dfs(mapa, count, u);
        for (auto& x : mapa) {
            int nivel = x.first, u = x.second.first, v = x.second.second;
            int a = lca(u, v);
